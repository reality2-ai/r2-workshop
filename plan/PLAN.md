...
| 6 | BLE bootstrap FSM | ✅ | Ported to ESP32-S3 with NimBLE; sensor advertises R2-BEACON with class string `nz.ac.auckland.rocker` (FNV-1a-32 hash `0x624C47BC`; locked at SPEC-R2-WORKSHOP-DASHBOARD §6.3 + SPEC-R2-WORKSHOP-ENSEMBLE §2.1), receives signed `#wifi_offer`, persists in NVS. Retired `wifi_config.toml`. Bench-validated end-to-end including the settle-after-cycle delay (`lwip_bind_pre_init_hangs` memory + `get_active_sensor_ips` loopback filter). Class rotated from `nz.ac.auckland.workshop.sensor` (`0xE6C6AFCD`) on 2026-05-28 — `workshop` is now the template name, `rocker` is the deployment. |
