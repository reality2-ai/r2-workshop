...
| 13 | bool | `sd_mounted` — authoritative SD-presence flag: `true` if the firmware mounted an SD card at boot (`sd::try_mount()` returned `Some`), `false` if it tried and failed (no card, wiring fault, FAT error). Boot-time value; runtime hot-swap is not signalled (the next reconnect's announce reflects it). The webapp uses this *instead of* inferring from `/api/data/list` responses (the inference falsely reported "present" when a list call returned 200 with empty files on a sensor whose SD wasn't actually mounted). Required from firmware v0.3.1+. Older firmware omits the key and receivers MUST treat absence as **unknown** — render an em dash, never default to "present" or "absent". |
...
| 2026-05-31 | 0.3.2 | Announce key 13 `sd_mounted` (bool) added — authoritative SD-presence flag, replacing the webapp's previous optimistic inference from `/api/data/list` responses (which falsely reported "present" on sensors whose SD wasn't wired). Required from firmware v0.3.1+; older firmware omits the key and receivers MUST render "unknown" / em dash. |
