...
impl Sender {
    ...
    fn dispatch_inbound(
        ...
        match event_hash {
            ...
            EVT_DASH_CAPTURE_EVENT_MARK => {
                ...
                // ignore otherwise. No capture.state re-emit —
                // the sensor stays in its current state.
    ...
    /// BLE beacon/L2CAP → SPI bus + ADXL355 + SD mount + ring recovery
    /// (all done in `Sender::new`, before `run`). That is the meaningful
    /// "this image is not a brick" signal.
    ///
    ...
    fn confirm_image_valid(&mut self) {
        let probe = match self.adxl.as_mut() {
            Some(adxl) => match adxl.read_xyz_lsb() {
                Ok(_) => "adxl self-test OK",
                Err(_) => "adxl self-test errored (transient) — continuing",
            },
